...
def data_load(data_dir=data_dir):
    data = {}
    ...
    for k1, v1 in data.items():
        entity_list.append(k1)
        for k2, v2 in v1.items():
            entity_list.append(k2)
            triples.append([k1, 0, k2])
            for k3, v3 in v2.items():
                entity_list.append(k3[len(k2) + 3:])
                triples.append([k2, 1, k3[len(k2) + 3:]])
                for k4, _ in v3:
                    entity_list.append(k4)
                    triples.append([k3[len(k2) + 3:], 2, k4])
    ...
    for i in range(len(entity_list)):
        entity_id2name[i] = entity_list[i]

    for i in range(len(entity_list)):
        entity_name2id[entity_list[i]] = i
    for i in range(len(entity_list)):
        entity_list[i] = entity_name2id[entity_list[i]]
    ...
    #
    # for i in tqdm(range(len(valid_triples))):
    #     triple = valid_triples[i]
    #     h_ = entity_name2id[triple[0]]
    #     r_ = triple[1]
    #     t_ = entity_name2id[triple[2]]
    #     valid_triples[i] = [h_, r_, t_]
    ...
    print("Complete load. entity : %d , relation : %d , train triple : %d, , valid triple : %d" % (
        len(entity_list), len(relation_list), len(triples), len(valid_triples)))
    return entity_list, relation_list, triples, valid_triples
...
class TransE:
    ...
    def insert_test_data(self, file1, file2, triple_list):
...
if __name__ == '__main__':
    entitys, relations, triples, valid_triples = data_load()
    transE = TransE(entitys, relations, triples, embedding_dim=200, lr=0.01, margin=6.0, norm=1, C=0.25,
                    valid_triple_list=valid_triples)
    transE.data_initialise()
    transE.training_run(epochs=100, batch_size=1000, out_file_title="")
    # entity_emb = 'TransE_entity_200dim_batch1000'
    # relation_emb = 'TransE_relation_200dim_batch1000'
    # transE.insert_test_data(entity_emb, relation_emb, triples + valid_triples)
